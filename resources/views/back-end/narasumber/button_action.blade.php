@if ($narasumber->deleted_at)
    <form action="{{route('admin.manage-narasumber.destroy',$narasumber->id)}}" method="DELETE" class="d-inline">
        @method('PATCH')
        @csrf
...
@else
    <a href="{{route('admin.manage-narasumber.edit',$narasumber->id)}}" class="btn  btn-primary btn-flat btn-sm" data-toggle="tooltip"
       data-placement="top" title="edit"><span class="fa fa-edit"></span></a>

    <form action="{{route('admin.manage-narasumber.destroy',$narasumber->id)}}" method="post" class="d-inline"
