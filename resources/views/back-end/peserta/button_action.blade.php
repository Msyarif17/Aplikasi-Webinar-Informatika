@if ($User->deleted_at)
    <form action="{{route('admin.manage-peserta.destroy',$User->id)}}" method="DELETE" class="d-inline">
        @method('PATCH')
        @csrf
...
@else
    <a href="{{route('admin.manage-peserta.edit',$User->id)}}" class="btn  btn-primary btn-flat btn-sm" data-toggle="tooltip"
       data-placement="top" title="edit"><span class="fa fa-edit"></span></a>

    <form action="{{route('admin.manage-peserta.destroy',$User->id)}}" method="post" class="d-inline"
